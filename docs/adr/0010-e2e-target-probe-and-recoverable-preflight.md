...
2. 浏览器侧 `Target Probe` 是非权威诊断，不进入 Verdict。它使用与正式链路相同的系统 Chrome、一次性 Profile 和 Gateway。Runtime 按执行 lane 和同一 Run 的历史结果选择 `resource-closure`、`application-ready`、`dom-identity` 三档策略；全部为 preview-readonly 时以应用就绪为首选，严格闭包失败后的安全重试自动升级策略。SPA HTTP 资源仍采用最多 5 轮、256 项的显式来源 GET/HEAD 精确 URL 闭包发现。写请求、非显式来源和任意页面动作仍被拒绝；WebSocket/SSE/HMR 只作为长期连接诊断，不会自动扩张 Gateway 权限。
...
7. Probe 在返回前冻结最小诊断快照：URL/title、DOM、可见文本摘要、Console/pageerror、失败请求、未闭合资源、长期连接、资源计数、策略与 attempt。每次业务命令后的 `get-status` 将该事实发布为 JSON、Markdown 和 HTML 中间报告，并明确业务 Case 是否执行。
