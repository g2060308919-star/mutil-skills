...
const ReceiptSchema = z.object({
  ...
  attemptId: SafeIdSchema, terminalIntentDigest: DigestSchema, bindingDigest: DigestSchema,
...
const CheckpointCoreSchema = z.object({
  schemaVersion: z.literal('1.0.0'), attemptId: SafeIdSchema, terminalIntentDigest: DigestSchema,
...
const StoreSchema = z.object({
  schemaVersion: z.literal('1.0.0'), entries: z.array(CheckpointSchema).max(128),
}).strict()
...
export class RuntimeFullPlaywrightCheckpointStore {
  ...
  private constructor(input: {
    ...
    this.#store.initialize(canonicalizeJson({ schemaVersion: '1.0.0', entries: [] }))
    this.#now = input.now
  ...
  async put(input: {
    ...
    return await this.#store.runExclusive(async () => {
      ...
      const core = CheckpointCoreSchema.parse({
        schemaVersion: '1.0.0', ...input, createdAt,
      })
      ...
      snapshot.entries.push(checkpoint)
  ...
  #readTransaction(): z.infer<typeof StoreSchema> {
    try { return StoreSchema.parse(JSON.parse(this.#store.begin())) }
    catch (cause) {
      this.#store.rollback()
