...
function completeSnapshot() {
  ...
  const semantic = {
    ...
    'requirement-model': semanticArtifact(snapshot, anchor, 'requirement-model', '1.0.0', {
      modelRevision: 1, requirements: [{ reqId: 'REQ-1', revision: 1, title: 'App', actors: ['qa'],
        entities: ['app'], preconditions: [], rules: [{ ruleId: 'RULE-1', category: 'business',
          statement: '状态可修改并清理', sourceRefs: ['CLAUSE-1'], certainty: 'explicit',
          oracleIds: ['ORACLE-1'] }], states: [], transitions: [], observableOutcomes: [{
          oracleId: 'ORACLE-1', ruleId: 'RULE-1', statement: '清理后状态恢复',
          sourceRefs: ['CLAUSE-1'] }],
        applicability: [], sourceRefs: ['CLAUSE-1'], status: 'active' }], coupledDimensions: [],
    ...
    'coverage-universe': semanticArtifact(snapshot, anchor, 'coverage-universe', '1.0.0', {
      ...
      obligations: [{ obligationId: 'OBL-1', reqId: 'REQ-1', clauseIds: ['CLAUSE-1'],
