...
export async function installStableTelemetryRuntime(options: StableTelemetryRuntimeOptions = {}): Promise<string> {
  ...
  const sourceCliDirectory = options.sourceCliDirectory ?? (existsSync(join(currentCliDirectory, '../bin', 'telemetry-hook.js'))
    ? resolve(currentCliDirectory, '..')
    : resolve(currentCliDirectory, '../../dist/src'))
  const sourceHooksRoot = options.sourceHooksRoot ?? resolveHooksPackageRoot(currentCliDirectory)
  ...
  const destinationHooks = join(runtimeRoot, 'node_modules', '@mutil-skills', 'hooks')
  const markerPath = join(runtimeRoot, '.mutil-skills-telemetry-runtime')
  const backupCli = join(runtimeRoot, '.previous-cli')
  const backupHooks = join(runtimeRoot, '.previous-hooks')
  ...
  await recoverInterruptedRuntimeSwap(destinationCli, destinationHooks, backupCli, backupHooks)
  ...
  const stagingHooks = join(stagingRoot, 'node_modules', '@mutil-skills', 'hooks')
  try {
    await mkdir(stagingBin, { recursive: true, mode: 0o700 })
    await mkdir(join(stagingHooks, 'dist', 'src'), { recursive: true, mode: 0o700 })
    await copyFile(join(sourceCliDirectory, 'runtime', 'cli.js'), join(stagingCli, 'cli.js'))
    await mkdir(join(stagingCli, 'runtime'), { recursive: true, mode: 0o700 })
    await copyFile(join(sourceCliDirectory, 'runtime', 'cli.js'), join(stagingCli, 'runtime', 'cli.js'))
    ...
    await copyJavaScriptTree(join(sourceHooksRoot, 'dist', 'src'), join(stagingHooks, 'dist', 'src'))
    await copyFile(join(sourceHooksRoot, 'package.json'), join(stagingHooks, 'package.json'))
    await rm(backupCli, { recursive: true, force: true })
    await rm(backupHooks, { recursive: true, force: true })
    try {
      if (existsSync(destinationCli)) await rename(destinationCli, backupCli)
      if (existsSync(destinationHooks)) await rename(destinationHooks, backupHooks)
      await rename(stagingCli, destinationCli)
      await mkdir(dirname(destinationHooks), { recursive: true, mode: 0o700 })
      await rename(stagingHooks, destinationHooks)
      ...
      await rm(destinationHooks, { recursive: true, force: true })
      if (existsSync(backupCli)) await rename(backupCli, destinationCli)
      if (existsSync(backupHooks)) await rename(backupHooks, destinationHooks)
      throw error
    }
  } finally {
...
async function copyJavaScriptTree(sourceRoot: string, destinationRoot: string): Promise<void> {
  await mkdir(destinationRoot, { recursive: true, mode: 0o700 })
  for (const entry of await readdir(sourceRoot, { withFileTypes: true })) {
    const source = join(sourceRoot, entry.name)
    const destination = join(destinationRoot, entry.name)
    if (entry.isDirectory()) {
      await copyJavaScriptTree(source, destination)
...
async function recoverInterruptedRuntimeSwap(
  ...
  if (!existsSync(destinationHooks) && existsSync(backupHooks)) await rename(backupHooks, destinationHooks)
  if (existsSync(destinationCli) && existsSync(backupCli)) await rm(backupCli, { recursive: true, force: true })
  if (existsSync(destinationHooks) && existsSync(backupHooks)) await rm(backupHooks, { recursive: true, force: true })
}
...
async function isOwnedTelemetryRuntime(runtimeRoot: string): Promise<boolean> {
  ...
  try {
    const packagePaths = [join(runtimeRoot, 'node_modules', '@mutil-skills', 'hooks', 'package.json')]
    for (const packagePath of packagePaths) {
      const packageJson = JSON.parse(await readFile(packagePath, 'utf8')) as { name?: string }
      if (packageJson.name === '@mutil-skills/hooks' && existsSync(join(runtimeRoot, 'cli', 'bin', 'telemetry-hook.js'))) return true
    }
    return false
  } catch {
    return false
...
export async function removeStableTelemetryRuntime(homeDir = process.env.HOME ?? process.cwd()): Promise<void> {
  ...
  await rm(join(runtimeRoot, 'node_modules', '@mutil-skills', 'hooks'), { recursive: true, force: true })
  await rm(join(runtimeRoot, '.mutil-skills-telemetry-runtime'), { force: true })
}

function resolveHooksPackageRoot(currentCliDirectory: string): string {
  const candidates = [
    resolve(currentCliDirectory, '../../../'),
    resolve(currentCliDirectory, '../../'),
    resolve(currentCliDirectory, '../../../../node_modules/@mutil-skills/hooks'),
    resolve(process.cwd(), 'packages/hooks'),
  ]
  const root = candidates.find((candidate) => existsSync(join(candidate, 'package.json')))
  if (!root) throw new Error('Cannot locate the installed @mutil-skills/hooks package')
  return root
}
...
export async function installHooksCommand(args: readonly string[]): Promise<string> {
  ...
  // uninstallHooks may remove the key when the old command is the only command
  // it sees; re-running the idempotent install guarantees the new command and key
  // remain present after replacing a repository-local runtime.
  await installHooks({ runtime, command: stableExecutable })
