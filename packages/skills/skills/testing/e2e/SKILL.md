...
2. 缺少 Runtime 时，只提示用户显式安装精确 `0.4.3`。
...
缺失时只展示以下精确建议，不得自行执行：`npm exec --yes --package=@mutil-skills/e2e-runtime@0.4.3 -- repo-e2e install-runtime --version 0.4.3`。不得探测、导入或建议安装 Contracts、Engine、Authority、Gateway、Browser、Sanitizer、Report、Store 等低层包。
