...
`full-playwright` 只接受冻结且通过源码验证的 `full-playwright/v1` program。它可使用受控 `page/context/browser/request/expect/testInfo/state` 完成表单、键盘、checkbox、Popup、多 Context/页面和 hover/dblclick；所有 document、Popup、子页面与 API 请求都必须命中冻结的 Gateway request intent。JSON/binary/template Body 只能由 Runtime 从冻结 `networkRequestBodies` 渲染并覆盖调用方 options，JSON 使用 canonical bytes。Program 必须为每个 Oracle 冻结 `checkpointId`、`oracleId`、`expectedJson` 及 expected digest；执行时只调用 Host 注入的 `checkpoint()` 提交 actual，Host 复算 `actualJson`、摘要和 pass/fail，并把本次截图、DOM、URL、Trace 的 `evidenceIds` 绑定结果。每个冻结 checkpoint 恰好执行一次；未知、重复、缺失、失败或缺少 evidence refs 都使 Case 失败。主程序和 Cleanup 使用两个独立 Chrome 生命周期；Cleanup 必须释放租约并返回 `verified-clean`，再以独立页面 Reload 验证目标状态已恢复。Gateway freeze/finalize、签名审计、完整 Oracle checkpoint 结果、证据与 Authority terminal 回执全部完成后才允许返回 passed；Checkpoint 恢复只能协调已执行终态，不得重放浏览器写动作。

## 退出条件

每个可执行 Case 有唯一 terminal attempt 或结构化阻塞；每个计划 Oracle 都有唯一 `checkpointId`、`oracleId`、`expectedJson`、`actualJson`、status 与 `evidenceIds`；real/injection 分区完整；事件链和 Gateway 计数可验。
