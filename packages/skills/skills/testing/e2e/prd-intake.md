...
来源候选、PRD-ID 建议和无法判定的身份问题。标准输出为带完整 Clause Inventory 的 `prd-manifest`、`prd-diff`
...
冻结正文与附件 bytes，记录来源和读取结果；逐条登记 Clause 的稳定 ID、`sourceId`、原文、规范文本、摘要与 `sourceSpan`，并由 Runtime 复算唯一 `inventoryDigest`。让 Engine 生成稳定身份、Revision 和 diff；对显式
...
来源集合不可变，manifest 覆盖全部正文/附件和 Clause Inventory；Clause ID 唯一、来源存在、`sourceSpan` 有效、单条摘要与 `inventoryDigest` 均可复算。Revision 可复算且事件已验签；approved/rejected 必须有
kind、decisionId、status 和 subject digest 全部匹配的 receipt，pending 必须没有 receipt。

## 暂停条件

正文为空、附件不可读、Clause 丢失/重复/来源未知/区间无效/摘要漂移、PRD 身份冲突、Lineage 决定缺失/验签失败或 Contracts major 不兼容；保存
