...
3. 将契约原文、主 PRD 与 `necessary-dependency` 来源作为 `create-run` 文件来源提交，并为每个来源显式记录 `target`、`reference` 或 `necessary-dependency`。一个 Run 只能有一组目标边界；参考文档、竞品页面、示例站点和辅助资料即使被读取，也不得进入验收范围、Case coverage 或 verdict。URL 或文本来源必须先获得有界、可复读的本地快照，并在投影 `origin` 中保留原始 kind/ref；不得让 Runtime 在执行期读取漂移网页。主来源与依赖来源合计不得超过 Runtime 公布的 8 MiB 上限。
