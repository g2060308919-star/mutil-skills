...
依次调用 `projectCompilerInputFromArtifacts()`、受信确定性 Compiler、独立 Discovery Authority、真实本地 Playwright CLI 固定的 `test --list --reporter=json` 离线质量门、`prepareTrustedCompilerRun()`、Contracts 校验和 Engine readiness 审计。可恢复写回归还必须调用 `createTrustedCompilerControlledWriteLauncher()`、loopback Controlled Write Bridge 与 fresh-run launcher。
