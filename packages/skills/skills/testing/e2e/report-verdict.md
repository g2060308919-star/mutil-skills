...
报告标题、目标网站、PRD 来源、测试用例、风险说明和展示排序候选。唯一结论、reason、Metric 和 cannotClaim 必须来自 Engine `FinalReport`。状态必须区分环境阻断、未执行、业务失败与正式通过；阻断不能记成业务失败，未执行不能进入通过率分母，业务失败必须绑定真实 Oracle actual 与证据。
