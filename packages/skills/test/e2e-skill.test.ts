...
describe('E2E skill package', () => {
  ...
  test('E2E manifest 只声明一个可安装 Runtime Host 能力门', async () => {
    ...
    expect(manifest).toMatchObject({
      ...
      requires: [{
        ...
        whenMissing: {
          action: 'prompt-install', package: '@mutil-skills/e2e-runtime', version: '0.8.0',
          terminalState: 'environment-blocked', reasonCode: 'E2E_RUNTIME_HOST_UNAVAILABLE',
        },
      }],
      source: {
        url: 'https://github.com/g2060308919-star/mutil-skills/blob/v0.8.0/packages/skills/skills/testing/e2e/SKILL.md',
        rawUrl: 'https://raw.githubusercontent.com/g2060308919-star/mutil-skills/v0.8.0/packages/skills/skills/testing/e2e/SKILL.md',
        ref: 'v0.8.0',
      },
    })
  })
  ...
  test('默认使用系统 Chrome 与本地确认，不把 WebAuthn 登记设为必经步骤', async () => {
    ...
    expect(entry).toContain('@mutil-skills/e2e-runtime@0.8.0')
  ...
  test('0.7 工作流以高层编译和目标探测为主线，submit-candidate 只服务旧 Run', async () => {
    ...
    expect(entry).toContain('## Runtime 0.8.x 工作流契约')
