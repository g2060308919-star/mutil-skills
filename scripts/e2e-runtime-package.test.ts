  ...
  test.skipIf(!runPackedInstall)(
    ...
    async () => {
      ...
      for (const packageName of publishedPackages) {
        ...
        expect(manifest).toMatchObject({ name: packageName, version: '0.1.0' })
...
async function resolvePackageTarballs(directory: string): Promise<string[]> {
  ...
  return publishedPackages.map((packageName) => {
    ...
    const matches = files.filter((file) => file === `${expectedPrefix}-0.1.0.tgz`)
    if (matches.length !== 1) {
      throw new Error(`打包目录中 ${packageName}@0.1.0 的 tarball 数量必须为 1`)
    }
